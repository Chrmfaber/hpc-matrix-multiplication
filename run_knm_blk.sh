...
BS="2 4 8 12 16 24 32 64 128 192 256"
...
if [[ "$TTT" = "blk" ]]
  ...
  #echo "Testing for BLK with knm = $values and bs = $size now."
    MATMULT_COMPARE=0 ./matmult_c.gcc $TTT $values $values $values $size| grep -v CPU >> Data/Gcc485_O3small$TTT.$LOGEXT
  done
else
  #echo "Testing for $TTT wiht knm = $values now."
   MATMULT_COMPARE=0 ./matmult_c.gcc $TTT $values $values $values | grep -v CPU >> Data/Gcc485_O3small$TTT.$LOGEXT
fi
done
